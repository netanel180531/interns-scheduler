...
if run_button:
    ...
    SHIFT_HOURS = {'regular_weekday': 8, 'night_weekday': 16,
                   'regular_friday': 5, 'night_friday': 19, 'night_saturday': 24}
    ...
    for i in range(num_interns):
        for week in range(4):
            ...
            model.Add(sum(total_hours) <= 71.5)
    ...
    for i in range(num_interns):
        ...
        for d in range(NUM_DAYS):
            for s in SHIFT_TYPES:
                total_hours.append(intern_assigned[(i, d, s)] * SHIFT_HOURS[s])
        total_hours_per_intern.append(sum(total_hours))

    max_diff = model.NewIntVar(0, 500, 'max_diff')
    for i in range(num_interns):
        for j in range(i + 1, num_interns):
            diff = model.NewIntVar(-500, 500, f'diff_{i}_{j}')
            model.Add(diff == total_hours_per_intern[i] - total_hours_per_intern[j])
            model.AddAbsEquality(model.NewIntVar(0, 500, ''), diff)
